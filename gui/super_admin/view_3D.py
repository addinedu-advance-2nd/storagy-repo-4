...
class IMUVisualization(QWidget):
    ...
    def update_plot(self):
        ...
        if self.node_name == 'depth_camera':
            # 뎁스 카메라 데이터를 3D로 시각화
            self.ax.scatter(self.x_data, self.y_data, self.z_data, c='g', s=10)
        elif self.node_name == 'lidar':
            # LiDAR 데이터를 3D로 시각화
            self.ax.scatter(self.x_data, self.y_data, self.z_data, c='r', s=10)
...
# SensorSubscriber 클래스 (ROS 2 메시지 구독)
class SensorSubscriber(Node):
    def __init__(self, imu_visualization, node_name):
        super().__init__('sensor_subscriber')

        # IMUVisualization 객체를 전달받아 사용
        self.imu_visualization = imu_visualization
    ...
    def depth_callback(self, msg):
        ...
        self.imu_visualization.add_data(x, y, z)
